...
def train(cfg, train_loader, model, criterion, optimizer, scaler, epoch, num_batch,
    ...
    for i, (input, target, paths, shapes) in tqdm(enumerate(train_loader)):
        ...
        if num_iter < num_warmup:
            ...
            for j, x in enumerate(optimizer.param_groups):
                ...
                x['lr'] = np.interp(num_iter, xi, [cfg.TRAIN.WARMUP_BIASE_LR \
                                    if j == 2 else 0.0, x['initial_lr'] * lf(epoch)])
                if 'momentum' in x:
                    x['momentum'] = np.interp(num_iter, xi, [cfg.TRAIN.WARMUP_MOMENTUM, 
                                                                cfg.TRAIN.MOMENTUM])
