...
def train(args, hyp, train_loader, model, criterion, optimizer, scaler, epoch, num_batch,
         num_warmup, writer_dict, logger, device, rank=-1):
    """
    train for one epoch
    ...
    for i, (input, target, paths, shapes) in tqdm(enumerate(train_loader)):
        ...
        #print('time:{}'.format(intermediate-start))
...
def validate(epoch, args, hyp, val_loader, val_dataset, model, criterion, output_dir,
              writer_dict=None, logger=None, device='cpu', rank=-1):
    """
    validata
    ...
    save_dir = str(output_dir / 'visualization')
